...
void aw::InteriorGenerator::generateWalls(Realm *realm, const WallsParam &param) {
    ...
    std::uniform_int_distribution<int> directionSelector(0, 3);
    ...
    while (cells.size() < param.MaxCells) {
        ...
        int dir = directionSelector(m_generator);
        if (dir == 0) {
            ++cell_x;
        }
        else if (dir == 1) {
        ...
        if (!grid[cell_y][cell_x]) {
            ...
            if (cellCount >= param.MinCells) {
                if (earlyExitSelector(m_generator) == 0) break;
            }
